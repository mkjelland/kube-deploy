...
import (
	...
	"k8s.io/kube-deploy/cluster-api-bosh/cloud/bosh/manifest"
)
...
func TestInstanceGroups(t *testing.T) {
	manifest, _ := manifest.Parse(example)
...
func TestAddDeleteWorker(t *testing.T) {
	...
	group := "worker"
	pattern := fmt.Sprintf("%s_", group)

	name, err := manifest.AddWorker()
	if err != nil {
		t.Error(err)
	}

	if !strings.HasPrefix(name, pattern) {
		t.Errorf("expected %s to follow pattern %s", name, pattern)
	}
